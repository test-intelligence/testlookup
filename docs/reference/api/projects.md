...
Source: [backend/app/routers/projects.py:47](../../../backend/app/routers/projects.py#L47).
...
Source: [backend/app/routers/projects.py:78](../../../backend/app/routers/projects.py#L78).
...
```python
payload: ProjectCreate, db: AsyncSession=Depends(get_db), current_user: User=Depends(get_current_active_user)
```

Direct handler error branches (dependency/service errors can add others):

```python
HTTPException(status_code=409, detail=f"Project with slug '{payload.slug}' already exists")
```
...
Source: [backend/app/routers/projects.py:249](../../../backend/app/routers/projects.py#L249).
...
Source: [backend/app/routers/projects.py:172](../../../backend/app/routers/projects.py#L172).
...
Source: [backend/app/routers/projects.py:185](../../../backend/app/routers/projects.py#L185).
...
```python
project_id: uuid.UUID, payload: ProjectUpdate, db: AsyncSession=Depends(get_db), current_user: User=Depends(get_current_active_user)
```
...
Reset Default Qa Lead Password Endpoint
...
```python
project_id: uuid.UUID, db: AsyncSession=Depends(get_db), current_user: User=Depends(get_current_active_user)
```
...
```json
{
  "operationId": "reset_default_qa_lead_password_endpoint_api_v1_projects__project_id__default_qa_lead_reset_password_post",
  "parameters": [
    {
      "in": "path",
      "name": "project_id",
...
```python
{'user_id': str(user.id), 'email': user.email, 'username': user.username, 'password': password, 'project_id': str(project.id), 'actor_id': str(current_user.id)}
```
...
## POST `/api/v1/projects/{project_id}/reset`
...
```python
project_id: uuid.UUID, payload: ProjectResetRequest, db: AsyncSession=Depends(get_db), current_user: User=Depends(get_current_active_user)
```
...
```json
{
  "operationId": "reset_project_data_api_v1_projects__project_id__reset_post",
  "parameters": [
    {
      "in": "path",
      "name": "project_id",
  ...
  "requestBody": {
    "content": {
      "application/json": {
        "schema": {
          "$ref": "#/components/schemas/ProjectResetRequest"
        }
      }
    },
    "required": true
  ...
  "responses": {
    "200": {
      "content": {
        "application/json": {
          "schema": {
            "$ref": "#/components/schemas/ProjectResetResponse"
          }
        }
      },
      "description": "Successful Response"
