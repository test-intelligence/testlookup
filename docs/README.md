...
Narrative pages trace the principal workflows through routes, services, stores, workers and clients. [API contract gaps](reference/api-contract-gaps.md) explicitly list responses without a complete structured schema; their handler links remain the authority for fields not expressed in OpenAPI. [Verification](handoff/verification.md) records the actual validation performed.
