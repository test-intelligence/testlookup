...
# Unlike `clean` above, this deletes nothing that is not regenerated by the
# next test run, so it does not gate on CONFIRM. Run it before a homelab
# deploy: require_clean_build_inputs in homelabsetup/deploy-homelab.sh refuses
# to build while ignored build inputs exist under backend/frontend/mcp/client.
clean-scratch: ## Remove pytest basetemp roots and tool caches (DRY_RUN=1 to list only)
	@DRY_RUN="$(DRY_RUN)" bash scripts/clean_scratch.sh
...
benchmark-pipeline: ## Baseline the AI pipeline from recorded telemetry (needs runs in the DB)
	@echo "==> Aggregating AI pipeline baseline from agent_stage_results..."
	$(DOCKER_COMPOSE) exec backend python /app/benchmarks/pipeline/collect.py \
		--days $(or $(DAYS),30) \
		--output /app/benchmarks/results/pipeline_baseline.json
	@echo ""
