...
def _stub_over_real(name: str, **overrides) -> types.ModuleType:
    ...
    A stub built from a fixed attribute list only works while the routers under
    test were already imported with the real module by an earlier test file.
    Run alone (or first), ``app.routers.runs`` is imported fresh under the stub
    and dies on the first name nobody listed (``resolve_release_query_scope``).
    Must be called BEFORE any stub is installed, so the real module binds real
    dependencies.
    """
