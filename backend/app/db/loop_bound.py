...
A Celery worker child builds a new event loop for its first task, and again
after a task is interrupted (``worker/loop_runner.py``; before re-audit M1 it
built one for every task). Any client cached at module scope that captured
the previous loop is unusable on the next one, and says so with
...
One function, called by the loop runner both wrappers share, so a newly cached
client is added in a single place. Anything registered here must be safe to
call with no running
...
def reset_loop_bound_clients() -> None:
    ...
    owns it* when the worker loop is torn down (``dispose_engine_for_loop``),
    which is the
    ...
    before a new loop exists, so it cannot await anything: nulling ``_pool``
    ...
    ``worker.loop_runner``'s teardown, which runs on the loop that owns
    those sockets.
