...
class AnalysisAgent(BaseAgent):
    ...
    def _engine_label(analysis: dict) -> str:
        ...
        """
        return engine_label(analysis)

    def _validate_confidence(self, analysis: dict) -> dict:
        """Validate and adjust the confidence score based on evidence quality.

        The rules themselves live in ``services/confidence_validation`` because
        they are the product's policy about what a claim is worth, not a
        pipeline detail — and ``POST /api/v1/analyze`` persists confidences too.
        While this logic was a private method here, that endpoint stored the
        engine's raw number and overwrote the pipeline's validated one.
        """
        return validate_confidence(analysis)
