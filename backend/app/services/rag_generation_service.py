...
async def grounded_generate(
    ...
    try:
        ...
        if was_redacted:
            logger.info("Redacted sensitive content from prompt for batch %s", batch.id)
...
async def _call_llm_generate(prompt: str, config: Optional[dict]) -> list[dict]:
    ...
    try:
        ...
        logger.warning("LLM generation timed out after %ds, returning stub cases", llm_timeout)
        return _stub_generated_cases(effective_prompt)
    except Exception as exc:
