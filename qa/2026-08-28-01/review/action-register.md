...
| QA-005 | P1 | Blocked | GitHub/release | GitHub CLI token is invalid and Git has no usable HTTPS credential | `gh auth status`; `git ls-remote` credential failure | Re-authenticate GitHub CLI or provide a valid Git credential, then publish the reviewed branch and open the review/merge path | Remote branch/PR exists and required checks pass | User credential action |
