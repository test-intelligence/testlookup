...
- New **`--since <ISO timestamp>`** overrides the day window with an explicit bound. A malformed value is **rejected**, not ignored — falling back to `--days` would silently produce exactly the blended baseline this prevents. The bound is recorded in `window.since`, and the rendered header now says `since <ts>` instead of claiming "over 30 day(s)", which is how a narrow baseline gets misread as a monthly one.
- Regenerated from the #805 deploy boundary (`--since 2026-08-23T06:05:00Z`): **50 runs, 31 deep + 19 investigation**, `sufficient_samples: true`.

  | Metric | All-time (1,329 runs) | Post-fix (50 runs) |
  |---|---|---|
  | degraded runs | 10.6% excl. incident | **0%** |
  | summary parse failures | 73 | **0** |
  | claims with evidence | 100% | **100%** (88 claims) |
  | shared-evidence-bundle rate (F-16) | 10.1% | **0%** |
  | narrative citation rate (F-3) | 8.9% | **74.2%** |
  | fabricated evidence IDs | 0 | **0** |

- **What these 50 runs are, stated plainly.** 31 of them are the recovery re-runs of previously-trapped pipelines, which restore stages from checkpoint rather than executing them. So **latency and token figures here are not representative of fresh work** — the summary stage made only 20 LLM calls across all 50 runs, because restored stages skip execution. The grounding figures *are* meaningful: those reports were regenerated by current code.
- The citation rate moving 8.9% → **74.2%** and the shared-bundle rate 10.1% → **0%** is what F-3 and F-16 actually do. The all-time file dilutes both with months of pre-fix history; that is the point of keeping a separate post-fix reference rather than quoting one number.
...
## 2026-08-22 — Dismissing the first-run guide on one empty project hid it on every new project

- The dashboard's first-run guide (`FirstRunGuide`) is a **per-scope** empty-state helper: `OverviewPage` shows it only when the active scope — a project, or All Projects — has no runs at all. But its dismissal was written to a **single browser-wide** localStorage flag (`tl_first_run_guide_dismissed`). The show was per-project; the hide was global.
- The consequence for a self-hoster, whose workflow is multi-project by design: dismiss the guide once on your first empty project (say, after `make quickstart`), and the same getting-started help is **silently suppressed on every genuinely new, still-empty project you create later** — the exact moment a fresh project most needs it. There was no way back to it short of clearing site data.
- A second, coupled defect: the guide's dismissed state was read **once at mount** into component state, and the route does not remount when you switch projects in the top bar (its `<Route>` key is the path, not the project). So even the show/hide could go stale across an in-session project switch.
- **Dismissal is now keyed on the active scope** — `tl_first_run_guide_dismissed:<projectId>` (or the All-Projects sentinel) via `firstRunDismissKey`. `OverviewPage` reads it fresh each render through `isFirstRunGuideDismissed(activeProjectId)`, so a project switch re-evaluates; a small bump-counter re-render makes a dismiss click take effect immediately without a reload.
- The legacy browser-wide flag is **deliberately not honored** as a universal suppressor — reading it that way would preserve the bug. A user who globally dismissed a prior build sees the guide once more on each still-empty scope (an empty dashboard, where it is helpful) and can re-dismiss per project. Blocked-storage / privacy-mode browsers are guarded: a throwing `localStorage` reports not-dismissed rather than crashing the dashboard.
- The dismissal helpers (`FIRST_RUN_DISMISS_KEY`, `firstRunDismissKey`, `isFirstRunGuideDismissed`, `dismissFirstRunGuide`) live in the pure `firstRunSteps.ts` module, not the component file, so `FirstRunGuide.tsx` still exports only its component — no new `react-refresh/only-export-components` warnings.
- Regression tests (11 new): 5 unit tests pin the per-scope semantics (namespaced key, isolation between projects, the legacy flag no longer suppressing, falsy-scope safety); 4 `OverviewPage` wiring tests prove the empty-project guide writes the project-scoped key on dismiss (not the bare one), stays hidden only for the project actually dismissed, still shows when a *different* project was dismissed, and ignores the legacy browser-wide flag. Mutation-checked: reverting the read to the bare global key fails 6 of them.
...
- The consequence is a **permanently silenced test**. The row stays in `_ACTIVE_QUARANTINE_STATES` past its own expiry for ever: the CI manifest keeps publishing it so `testlookup ci-verdict` keeps exiting 0 for its failures, ingestion keeps tagging it `quarantined`, and the release gate keeps excluding it from scoring. A test that has been quarantined twice cannot block a build — not when it regresses, and **not when it is fixed**, because the recheck that would have released it never runs.
- Measured against a real Postgres driving the real beat chain, with the clock advanced 14 days per night. A test that is genuinely flaky on night 1 and fixed from then on: **night 2 — 12 consecutive passes, `rechecks_scheduled: 0`, still `RE_QUARANTINED`; night 3 — 24 consecutive passes, `rechecks_scheduled: 0`, still `RE_QUARANTINED`.** With the fix, night 2 sweeps the row and `run_recheck_cycle` releases it in the same pass (`released: 1`).
- **The model always said this should loop.** `postgres.py` documented `flip-rate still high -> [RE_QUARANTINED] -> QUARANTINED (new window)`; only the query disagreed, and a query is not visible in a docstring. That comment was itself wrong in a second way — a re-quarantine *keeps* the status `RE_QUARANTINED`, it never writes `QUARANTINED` again — so the diagram described a transition the code has never made. Both the diagram and the beat task's docstring (which promised "three passes" and then listed four) now describe what actually happens.
- The guard checks the **class, not the tuple**: every non-terminal status the nightly chain writes must be one a sweep *in that same chain* can move a row out of. A live state nothing can leave is a trap, and adding one tomorrow without its selector fails the test.
- Two scope calls decide whether that guard can see anything, and the first version of it got the second one wrong. `update_quarantine_stability` reads the active states but runs at run finalization and releases nothing unless the off-by-default `auto_promote` policy is on, so it is not counted. And **selecting a status is not escaping it** — `mark_stale_quarantines` selects every active status and only stamps a notification timestamp, leaving the row exactly as stuck; counting that read let the guard survive both mutations of the very defect it exists to kill. A status counts as escapable only when the function selecting it also writes some status.
...
- `apply_async` never raises there. It calls `ResultConsumer.on_task_call` → `consume_from`, which subscribes to the result pubsub channel and reconnects through `kombu.utils.functional.retry_over_time`; nothing is raised, so the `except` never runs. Captured with `faulthandler.dump_traceback_later`, since a hang produces no traceback of its own.
- `tests/conftest.py` now neutralises that subscription, which restores the assumption every dispatch call site is written against — and which `backend/CLAUDE.md` states outright, that the local suite needs no live services. The dispatch falls through to the broker publish, which does raise promptly. Bounding `retry_policy` instead does **not** work: celery reads a nested `retry_policy` key and the consumer holds its own connection. That was measured, not assumed — net removed and patches removed, the run still hangs past 90s; with the net alone it finishes in 9.5s; with the test's explicit patches, 1.6s.
- The test also patches both dispatch points explicitly, matching what the service-layer tests already do, and a regression test pins the conftest hook so deleting it fails loudly instead of restoring an undiagnosable hang.


## 2026-08-21 — The full project reset deleted nothing at all

- `_FULL_RESET_TABLES` deleted `test_suites` **before** `canonical_test_cases`, which holds a NOT NULL `ON DELETE RESTRICT` FK to it — deliberately, so a suite still holding cases cannot be dropped out from under them. Canonical rows survive the `test_runs` delete (their run pointers are `SET NULL`), so on any project that had ever ingested a run, Postgres aborted the statement and rolled the transaction back. The `full` reset deleted **nothing** — including the `test_runs` rows removed earlier in the same transaction. Reproduced on PostgreSQL 16 against the real FK topology: `ERROR: update or delete on table "test_suites" violates foreign key constraint "canonical_test_cases_test_suite_id_fkey"`, then `ROLLBACK`, with all four tables still populated. Deleting children first commits and leaves all four at zero.
- The existing unit tests could not have caught this: they mock `db.execute`, so no constraint is ever evaluated. The new guard checks the ordering statically for every pair — no entry in `_FULL_RESET_TABLES` may be deleted before a table holding a blocking FK to it — and a companion test fails if that `RESTRICT` ever softens, since the ordering rule would then be passing vacuously.

## 2026-08-21 — The project reset deletes the suite ledger it promises to delete

- The Project Data reset modal lists exact tables under **"This will permanently delete:"** before an irreversible ADMIN-only wipe. For `full` it named `suite_memberships` — and `_FULL_RESET_TABLES` never deleted it. Nothing cascaded to it either: a membership names its suite by `suite_name` (a `String`, not a FK to `test_suites.id`), its three `test_runs` pointers are all `SET NULL`, and its only `CASCADE` is on `projects.id`, which a reset never deletes because the Project row is a documented keep.
- The survivors were not inert. `suite_sync_service` still writes this table on every ingest — migration 0075 deferred that move to a Phase 2 that has not landed — so on the first post-reset ingest every pre-reset fingerprint took the "already seen" branch: counted unchanged or restored rather than added, `first_seen_run_id` never assigned and NULL from then on, and any survivor absent from that run marked `needs_review` and moved to the `-deleted` bucket. A project advertised as green-field would report restoring and deleting tests it had never ingested.
- Both `suite_memberships` and its paired `suite_membership_events` are now deleted explicitly, and the modal names both. Neither is one of the four append-only audit tables (`settings_audit_log`, `access_audit_logs`, `test_case_audit_logs`, `identity_events`), so the audit-write-discipline boundary is unchanged.
- The guard compares the whole rendered table list against the whole delete set rather than pinning the one table name, and computes cascade reachability from the models instead of trusting the `(CASCADE)` annotations the page writes beside a name. That reachability has to be transitive — the page's `ai_analysis (CASCADE)` claim is true only via `test_cases` — and rooted at each mode's real delete set, since `full` also deletes `test_suites` outright. It fails in both directions: a table promised but not deleted leaves data the user was told was gone, and a table deleted but not promised means the blast radius grew silently, which for a destructive action is the worse of the two.
...
- Skipped providers now record `status: "skipped"` with the reason and a current timestamp. The seven never-configured integrations (jira, splunk, github, ocp, slack, teams, smtp) previously had no row at all, so only 2 of 9 appeared on the page; they are now visibly not-monitored rather than invisible. `IntegrationHealthPage` already had a `skipped` badge style — nothing had ever been able to render it.
