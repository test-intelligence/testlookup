...
- 4 GB RAM / 2 vCPU / 20 GB disk (core mode); 8 GB / 4 vCPU (full mode with LLM)

TestLookup ships **three run modes**. Pick the one that matches what you want to try.

### Mode 1 -- Core (no LLM, ingest your own data)

Everything except the local LLM. Analysis uses the rules engine (deterministic, no network calls). About 5 minutes on a fresh machine.

```bash
git clone https://github.com/anandtopu/testlookup.git
cd testlookup
cp .env.example .env           # edit secrets + passwords before starting
...
### Mode 2 -- Full (with local LLM for AI-assisted triage)

Core stack plus a local Ollama instance and ChromaDB vector store. About 10 minutes including the model pull.

```bash
git clone https://github.com/anandtopu/testlookup.git
cd testlookup
cp .env.example .env
make dev-llm                   # boots everything including ollama + chromadb
docker compose exec ollama ollama pull qwen2.5:7b
```

The `make dev-llm` target applies the `local-llm` Docker Compose profile which starts the `ollama` and `chromadb` services. **Without this profile, those containers are not started** -- this is the quickstart inconsistency fixed in v0.1.0.
...
Database migrations run automatically when the backend container starts (`alembic upgrade head`). No manual migration step is needed.
...
### Connect the MCP Server (AI Assistant)
