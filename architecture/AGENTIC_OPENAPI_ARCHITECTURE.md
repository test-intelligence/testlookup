...
| 3 | User-customizable workflows (steps, branching, tool usage) | **PARTIAL** — E3.1 stores strict, versioned project definitions and exposes CRUD, validation, fork, and publish endpoints; built-ins are read-only templates. E3.2 semantically validates and compiles those documents into LangGraph graphs, with built-in topology parity. Runs still select the hand-built graphs until E3.3 binds a published `workflow_id@version`. | `models/postgres.py` (`WorkflowDefinition`), `services/workflow_definition_service.py`, `agents/workflow_compiler.py`, `routers/workflows.py` | Epic E3 |
