...
- **E3.2 (L)** `WorkflowCompiler`: registry validation, dependency closure, cycle rules, capability-permission ≤ mode, step tools ⊆ config allowlist, typed JSON condition AST evaluator with limits and enum checks (fuzz tests), emits `StateGraph`; three built-ins expressed as definitions and diff-tested against today's compiled graphs. **(T17/E3.2 shipped)** `agents/workflow_compiler.py` rejects unknown or non-top-level capabilities, dependencies that cannot complete before their consumer, ordinary-edge cycles, invalid reviewer targets, project config/mode/tool violations, and non-exhaustive ordered branches. Its JSON condition AST accepts only bounded boolean composition and typed comparison/count/null/boolean/else leaves over a published state projection or frozen config references; enum values, depth 6, 32 leaves, and `in` lists of 50 are enforced before publication. Explicit back-edge loops use compiler-owned counter nodes and stop exactly at `max_iterations`. The workflow validate endpoint now reports semantic results, and evaluation/publish fail with 422 before G4 work when compilation is invalid. Offline, deep, and live definitions describe the current conditional and parallel topologies, with diff tests against the hand-built graphs. **Code-truth correction:** finalization is a fenced post-graph transaction, not a registered agent node, so terminal paths compile to LangGraph `END`. **Deviation:** disabled capabilities remain structurally compilable because the live deep template contains feature-gated, default-disabled `defect_commander`; permission is enforced whenever a config enables execution. **Gap:** E3.3 still owns selecting `workflow_id@version`, freezing its hash, and executing the emitted graph; reviewer-input binding and shared loop/model budget propagation arrive at that runtime boundary.
- **E3.3 (S)** Runs record `workflow_id@version` + plan hash; checkpoint restore refuses across versions. **(T17/E3.3 shipped)** The pipeline trigger accepts an additive workflow id/version selection and resolves only immutable published versions, defaulting compatibly to `offline@1`. Run creation freezes the exact definition, canonical definition digest, compiled plan digest, deadline, retry/review policy, and safe project agent-config documents in `execution_metadata`; terminalization preserves that authority. Both first execution and same-id resume compile the frozen snapshot, while resume and prior-run checkpoint restore refuse definition, plan, or version mismatches. Step-id executor binding supports repeated named capabilities with task-local durable stage identity, and reviewer steps receive the declared upstream outputs, test/cluster references, numeric run facts, and shared pipeline budget context. **Deviation:** no columns or migration were added because `execution_metadata` is already the immutable replay-authority store. **Gap:** reviewer artifact references remain empty until workflow state exposes the canonical artifact-id projection; E3.4–E3.5 remain.
...
- **E9.5 (M)** G4 workflow evaluation: replay cache keyed by `(agent_id, prompt_version, input_hash)`, `unmeasured` on miss, coverage metric, `POST …/workflows/{id}/evaluate`, publish refusal unless `accept_regression` + reason. Migration 0182+: `workflow_replay_corpus`. **(E9.5 shipped)** The project-scoped dry-run caches stage outputs under the exact three-part key and compares candidate definitions with the latest 20–100 runs for plan-verification pass rate, degradation, reviewer rejection, cost, and latency. Coverage below 0.80 or fewer than 20 runs is `insufficient_samples` and remains publishable with coverage stored on the definition; a measured regression is refused unless a QA Lead records `accept_regression` and a non-empty reason. Draft edits clear stale evidence, publish triggers an evaluation when none exists, and a Prometheus gauge exports the latest coverage. **Deviation:** migration 0184 was used because origin/main already assigns 0182 and 0183. The stage schema does not retain a generic raw input document, so `input_hash` uses the stable project/test-run/step identity and `prompt_version` is the checksum of the run's frozen prompt-version map. **Gap:** exact cache misses stay `unmeasured`; optional local live inference remains blocked on E3.2's generic compiler/input binding.
