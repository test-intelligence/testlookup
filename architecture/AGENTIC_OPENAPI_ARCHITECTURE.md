...
| 3 | User-customizable workflows (steps, branching, tool usage) | **PARTIAL** — E3.1 stores strict, versioned project definitions and exposes CRUD, structural validation, fork, and publish endpoints; built-ins are read-only templates. Runtime graphs are still compiled from Python until E3.2/E3.3 compile and bind these definitions. | `models/postgres.py` (`WorkflowDefinition`), `services/workflow_definition_service.py`, `routers/workflows.py` | Epic E3 |
