...
- **E6.2 (S)** Self-consistency claim extraction (family 3) on SLM. **(T16 shipped)** The reviewer re-asks the reviewed step's resolved SLM/LLM tier to extract reference ids and numeric facts, strips hidden-reasoning keys before prompt construction, and diffs the typed response against the authoritative evidence snapshot. Missing endpoints, malformed responses, and exhausted call budgets fail into a warning check and cap the verdict at `pass_with_flags`.
...
- **E6.4 (S)** Supervisor handling of `retry/reject/pass_with_flags`; shared `step_llm_budget`; mutation-tested checks (orphan ids, contradicted numbers, unsupported claims). **(T16 shipped; M13 persistence hardening shipped)** A deterministic supervisor revalidates `ReviewVerdictV1` fail-closed and emits typed `continue`, `retry`, or `finalize` instructions. Retry names the reviewed steps and pins `tier_override=llm`; a second retry or exhausted budget becomes `finalize`, `failed`, `validation_failed`; flags continue with `requires_human_review=true`. `StepLLMBudget` is serializable and its initial ceiling equals `min(run calls remaining, max_escalations + max_iterations)`. Family-5 blocking findings emit a `reviewer_incident` decision event. Custom runtime review flags now prevent automatic pass, create an evidence-bound human review subject even without a report-producing stage, and persist the verdict, supervisor route, and budget snapshot. Regression and mutation checks cover orphan ids, contradicted numbers, unsupported claims, budget exhaustion, one-retry handling, and invalid supervisor routes. **Code-truth correction / gap:** the earlier shipped note overstated production sharing: ModelRouter accepts a supplied budget, but Summary and Root Cause do not yet pass the graph's reviewer budget, and the reviewer `tier_override=llm` is emitted but not yet applied to the retried producer. Those are open M13 blockers.
