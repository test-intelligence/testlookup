...
- **E6.3 (M)** Second-model agreement (family 4) with heterogeneous model preference; config `review.second_model_check`. **(T16 shipped)** When enabled alongside the auto-reviewer, a distinct configured LLM returns a bounded agreement score plus unsupported and blocking claims. Agreement below 0.7 or non-blocking unsupported claims pins human review; a blocking unsupported claim requests the single supervised retry and rejects after that retry. The generator and reviewer prompt versions are independent. **Deviation:** today's strict config has one LLM endpoint, so the reviewer guarantees a different provider/model identity and uses a different family when that endpoint supplies one; selecting among multiple LLM-family candidates remains a future additive config change.
- **E6.4 (S)** Supervisor handling of `retry/reject/pass_with_flags`; shared `step_llm_budget`; mutation-tested checks (orphan ids, contradicted numbers, unsupported claims). **(T16 shipped)** A deterministic supervisor revalidates `ReviewVerdictV1` fail-closed and emits typed `continue`, `retry`, or `finalize` instructions. Retry names the reviewed steps and pins `tier_override=llm`; a second retry or exhausted budget becomes `finalize`, `failed`, `validation_failed`; flags continue with `requires_human_review=true`. `StepLLMBudget` is serializable, its initial ceiling equals `min(run calls remaining, max_escalations + max_iterations)`, and both ModelRouter escalation and the reviewer supervisor consume the same supplied object. Family-5 blocking findings emit a `reviewer_incident` decision event. Regression and mutation checks cover orphan ids, contradicted numbers, unsupported claims, budget exhaustion, one-retry handling, and invalid supervisor routes. **Gap:** E3 has not yet shipped custom workflow compilation, so the reviewer returns attach-ready supervisor instructions; E3 must pass the shared budget through graph edges and apply loop iteration accounting without changing the public state machine.
