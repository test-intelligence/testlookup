...
- **E9.5 (M)** G4 workflow evaluation: replay cache keyed by `(agent_id, prompt_version, input_hash)`, `unmeasured` on miss, coverage metric, `POST …/workflows/{id}/evaluate`, publish refusal unless `accept_regression` + reason. Migration 0182+: `workflow_replay_corpus`. **(E9.5 shipped; M13 hardening shipped)** The project-scoped dry-run caches stage outputs under the exact three-part key and compares candidate definitions with the latest 20–100 runs for plan-verification pass rate, degradation, reviewer rejection, cost, and latency. Coverage below 0.80 or fewer than 20 runs is `insufficient_samples` and remains publishable with coverage stored on the definition; the same visible verdict now applies when cached step outputs cannot measure branch, join, loop, or reviewer routing. A measured regression is refused unless a QA Lead records `accept_regression` and a non-empty reason. Draft edits clear stale evidence, publish triggers an evaluation when none exists, and a Prometheus gauge exports the latest coverage. Evaluation refuses published versions. Publish requests bind the selected version and canonical definition digest; stale requests return 409, while an exact repeat is read-only. **Code-truth correction:** `architecture/WORKFLOWS.md` incorrectly said insufficient samples could not publish, while section 11.2 and the live gate allow publication but forbid project-default selection; the guide now matches code and this architecture. **Deviation:** migration 0184 was used because origin/main already assigns 0182 and 0183. The stage schema does not retain a generic raw input document, so `input_hash` uses the stable project/test-run/step identity and `prompt_version` is the checksum of the run's frozen prompt-version map. **Gaps:** exact cache misses stay `unmeasured`; optional local live inference and project-default enforcement remain open.
