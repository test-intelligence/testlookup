...
| 3 | User-customizable workflows (steps, branching, tool usage) | **EXISTS** — strict versioned project definitions support semantic validation, bounded branches and loops, project tool/mode policy, evaluation, immutable publication, and execution by published `workflow_id@version`. The project editor supports fork/edit/preview/validate/evaluate/publish. Built-ins are read-only and guarded against compiler topology drift. | `models/postgres.py` (`WorkflowDefinition`), `services/workflow_definition_service.py`, `agents/workflow_compiler.py`, `routers/workflows.py`, `frontend/src/pages/WorkflowEditorPage.tsx` | Epic E3 complete |
...
| `WorkflowCompiler` | orchestration | EXISTS | `agents/workflow_compiler.py` | Validates a definition against the registry (dependencies satisfied, policy-compatible config, no cycles except explicit loops with max_iterations) and emits a `StateGraph`. The three built-in definitions are diff-tested against the live `_build_offline/deep/live_graph` topologies. |
...
| `GET/PUT /api/v1/projects/{project_id}/agent-configs/{agent_id}` | Per-agent config (§4). **Supersedes** `…/agent-policies/{agent_id}`: migration 0187 moved both legacy rows into `agent_configs`; deprecated legacy GETs remain read-only aliases for one release and legacy PUTs return 405 | sync |
...
Ownership rule: `mode` has exactly one writable home, `agent_configs.mode`. Migration 0187 moved the former `AgentPolicy.mode` values into it and removed the legacy table/model. `tools.mutating_allowed` from revision 1 is absent because it is derivable from `mode` (memory: two modules, one rule). A guard asserts only `agent_config_service` writes `mode`; its baseline is empty. `agent_runs.mode` is a separate per-run ledger *record* of the mode the run executed under, not a policy; the guard treats it as read-only provenance.
...
```json
{
  ...
  "extensions": {"investigator": null, "fixer": null}
}
```

`extensions` is a strict compatibility document for the two pre-registry
agents. Only `agent_id=investigator` may carry its complete legacy run and
cluster budgets, promotion note, and server-owned shadow-run counter; only
`agent_id=fixer` may carry its runner, test globs, budgets, and schedule.
Registry capability configs cannot carry either extension. The fields contain
no provider endpoint or credential, and the canonical single-resource endpoint
accepts these two compatibility IDs without adding them to the registry-backed
...
Compiler rules: every `agent_id` must exist in the registry; every registry `dependencies` of a step must be guaranteed upstream before the consumer can run; loops require `max_iterations`; every terminal path reaches LangGraph `END`; a definition is immutable once `published` (edits create a new version); `config_ref` resolves only within the workflow's own project. Runs record `workflow_id@version` and the plan hash, reusing `compute_workflow_plan_hash`.

**Code-truth correction (E3.2).** There is no `agent.finalize.v1` capability in the registry. Pipeline finalization is the fenced post-graph transaction in `workflow.py`, so requiring a graph node called `finalize` would create a second writer and contradict the live runtime. E3.2 therefore compiles terminal workflow paths to `END` and leaves the single finalizer outside the graph.
...
| **Dead-letter queue** | existing `_send_to_dlq`; compatibility reader at `/api/v1/admin/maintenance/dlq` | `GET /api/v1/admin/dlq` inspects both Redis stores and `POST /api/v1/admin/dlq/{id}/replay` safely requeues exhausted Celery tasks. |
...
**Monitoring**
- Emit *and* assert emission (memory: declaration is not emission): `agent_invocations_total{agent,tier,status}`, `agent_run_transitions_total{from,to}`, `agent_retry_attempts_total{agent,reason}`, `review_requests_total{state}`, `model_escalations_total{agent,from,to}`, `llm_circuit_breaker_state{provider,endpoint}`. The circuit gauge is endpoint-scoped because one provider can expose independent healthy and unavailable endpoints (E5.4).
...
- **E8.1 (M)** Migration 0174: `review_requests` (with `kind`, `capability_id`, `requested_by`, `reason_code`, `notes`, `evidence_bundle_sha256`), `users.is_synthetic` backfilled from the QA-lead domain, project setting `allow_unreviewed_distribution`. Finalize creates requests for report-producing runs; supersede logic. **(shipped as migration 0175)** 0174 was taken by E7.4's `rerun_of`. Added beyond the listed columns: `pipeline_run_id`, `test_run_id` and `workflow_type` (the supersede scope), `superseded_by` (so history links to its replacement), `created_by`. One live request per subject via a partial unique index; superseded rows are kept. A newer run supersedes only *pending* requests for the same test run and workflow type; an accepted review is never overwritten. A report whose evidence hash changes after its review settled gets its review superseded (section 8.3). **(T3 shipped)** Migration 0188 adds nullable `agent_pipeline_runs.requested_by -> users.id ON DELETE SET NULL`; manual single, bulk, deep, config-changed rerun and direct-invocation paths persist the authenticated trigger, while automatic/outbox runs remain NULL and same-id resumes preserve the original requester. Finalize copies that durable value to `ReviewRequest.requested_by`. **Deviation:** no index was added because requester is copied during primary-key pipeline lookup and is not a filter. **(D3 shipped)** Migration 0189 records the manual Investigator requester and carries it into the stable investigation pipeline. A completed verdict stages a pipeline-run review with a canonical verdict hash in the same terminal transaction; automatic investigations keep NULL requester attribution.
- **E8.2 (S)** `routers/reviews.py`: list (project-scoped), get/accept/reject with `require_review_access`; JWT-only; refuse synthetic; separation of duties for `mode=act`; audit-log; `passed` transition wired; `notes` redaction. **(shipped)** Reject moves the run `completed -> failed` with `review_rejected: <reason_code>`; accept moves it to `passed`; both via `guarded_transition`, so a run that moved on refuses the decision (409). Accept/reject also require QA_LEAD (checked on the user's global role, not the project role). `require_review_access` is router-local and returns 404 to non-members; the authorization ratchet now scans `{review_id}`. **(T2 shipped)** The E7.4 and invocation retry routes now enforce the terminal review rejection; see E7.4. **(T3 shipped)** A requester cannot settle a review when its pipeline produced an action-ledger proposal whose hashed proposer identity resolves to `mode=act`; invocation and ordinary workflow runs both use the proposal-time config frozen in pipeline execution metadata. Missing proposer identity or frozen config on a legacy proposal fails closed. Read-only and suggest-mode reports are no longer blanket-blocked. **(M07 hardening shipped)** Settlement and supersession share a pipeline-then-review lock order; review creation locks the stable parent `TestRun` before testing or inserting a live subject, which serializes concurrent first inserts for the same scope under PostgreSQL `READ COMMITTED`; ordinary and Investigator finalizers acquire that parent before their pipeline/investigation child row, matching retention and project-reset cascade order; changed evidence always mints a new review id; the final locked settlement read refreshes the ORM identity map; distinct Investigator subjects do not supersede each other; generic parent-run envelopes exclude Investigator reviews; and a queued worker repeats the review-rejection refusal under the run lock. **Gap:** the §8.3 per-project read-only separation toggle, default on, is not implemented; read-only self-review is currently allowed until that setting ships.
...
- **E8.4 (M)** Distribution gates: PDF, digests, PR comment, webhooks, MCP results, CLI read the project setting; watermark; audit row per inclusion; release gate returns `PENDING_REVIEW` / `ADVISORY_*` values. Test: pending report ⇒ 409 on export; setting on ⇒ watermark text in PDF and audit row. **(slice 1 shipped)** Policy in `report_distribution_policy.py`; gated so far: report PDF export (`include_unreviewed` for QA_LEAD+), public share links, and the release-readiness value (`PENDING_REVIEW` / `ADVISORY_*`, model value kept in `draft_recommendation`; synthesized quick-looks, human overrides and accepted reviews unchanged). Rejected and superseded reports are never distributed as drafts. **Rollout:** enforcement sits behind `REVIEW_GATE_ENFORCED` (default off), because runs finished before E8.1 have no accepted reviews and enforcing on merge would stop files and flip CI gates for every project at once; while off, would-be refusals are audited as `ai_report.distribution_would_refuse`. **Remaining slices:** notifications/digests, GitHub/GitLab comments + webhooks, MCP + CLI. **(slice 2 shipped)** AI summary notifications and event-driven digests are gated once before both fan-outs (withheld text replaced by an awaiting-review notice and the executive panel dropped when refused; DRAFT line under project opt-in; deterministic fallbacks never gated). The window analysis report's quoted release verdict gets the release-readiness projection. **(D3 shipped)** Every completed Investigator verdict now gets an exact pipeline review subject. Analysis-report and digest-attachment narrative excerpts resolve that exact subject (never the newest review for the whole test run), withhold pending/rejected prose under enforcement, watermark project-opted drafts, and audit each inclusion; deterministic cause, confidence, tally, and links stay visible. **(slice 3 shipped)** GitHub PR comment and GitLab MR note: AI failure-kind labels are gated (stripped when refused, a DRAFT note under project opt-in); the comment always posts, and the deterministic attribution summary is not gated. Audit rows from these read-only worker paths commit via `record_distribution_detached`. **Gap:** these comments post before the AI pipeline and are not re-posted on review acceptance. **Correction against main (PR #67):** `release_decision_webhook.py` now emits `release.decided` and applies the release review gate; the earlier no-emission finding is resolved. **(slice 4 shipped)** MCP: report tools end with `review_state` + disclaimer (`unknown` when absent); read-only `list_pending_reviews`; no accept/reject tool, with a parity test (pulled forward from E8.6). CLI: `reviews list/accept/reject` (accept/reject refused up front on API-key profiles); `intelligence show` prints review state + disclaimer to stderr. `reports pdf` unchanged (PDF carries the watermark; its route sends no review headers).
  **(M08 hardening shipped)** Immutable DecisionReport and Investigator excerpts bind historical authority to pipeline plus evidence hash; release readiness and `release.decided` bind to the persisted decision pipeline. Queued summary and release deliveries retain both pipeline and evidence identity, re-evaluate that exact review subject, persist the successful projection in history, and audit only a successful token-fenced delivery. Queued summaries freeze their original narrative bytes and AI provenance and no longer query Mongo when those bytes exist. Agent release events start only after immutable report publication, carry the critic's exact pipeline and evidence hash, and source decision fields from that report; valid human overrides do not depend on an agent report. Decision-row serialization prevents a delayed model event from following an override, while each concurrent override carries its own audit ordinal, timestamp and frozen council payload. Legacy rows missing evidence identity fail closed under enforcement, and legacy notification prefixes are rebuilt from deterministic run facts before egress. Terminal projections clear narratives and the original model verdict; authorized `allow_advisory=true` responses commit an actor-bound inclusion audit. **Deviation:** the shared homelab remains enforcement-off until the owner-configured release date. **Gaps:** dedicated SMTP/Slack/Teams/public-webhook/GitHub/GitLab sinks remain unavailable, and PR/MR comments are still not reposted after review acceptance.
- **E8.5 (S)** `/reviews` page + banners (use `add-page` skill). e2e: accept moves chip to `passed`. **(shipped)** `/reviews` Review Queue (single-project; QA-lead accept/reject with required reason code; read-only otherwise); `ReviewBanner` on the run intelligence page and the `/agents` summary panel (no banner for `not_applicable` or a missing block); `/agents` COMPLETED cards carry an `awaiting review` tag. **(T20 shipped)** Pipeline list and detail responses now carry an additive `review_summary` (`state`, `settled_at`) for the live pipeline-run report review, loaded in one batched query. Reviewer identity remains excluded. A PASSED card with an accepted review shows `· reviewed <time>` beside its unchanged four-value status chip; automatically passed non-report runs and rejected reports show no acceptance timestamp. **Deviation:** the time is a qualifier beside the chip so the chip text remains exactly one of the four E7.5 public statuses. **Gap:** reviewer-name hover is not available because pipeline API responses deliberately exclude reviewer identity; the review queue remains the identity-bearing in-app view.
...
- **E1.1 (S)** `GET /api/v1/agents/catalog[/{agent_id}]` from the registry; `sync_eligible` flag; per-agent input wrapper generation; guard `agents.catalog-schema-complete`; router-order test. **(shipped)** `services/agent_catalog.py` plus the two routes (authenticated; declared first in the agents router). `SYNC_ELIGIBLE` is a module-level registry set, not a `CapabilitySpecV1` field, so frozen plan snapshots are unchanged. Per-agent `<StageName>InvokeInput` wrappers use `payload: <model> | SubjectRef`. **(T6 shipped)** The catalog now publishes additive `default_tier` and sorted `escalation` trigger fields from module-level maps; `description` remains unpublished. **(T21 shipped)** Nine closed, versioned contracts resolve all 13 formerly label-only capability inputs, including the one shared by five Investigator hypothesis agents; every capability now reports `input_schema_resolved=true` and the guard baseline is empty. The models also resolve three output labels reused by the registry; 13 output schemas remain unresolved. **Code-truth deviation:** `agent_invoke.py` still deliberately refuses a concrete payload and executes only a stored `SubjectRef`, while workflow agents consume durable shared-state projections rather than these models directly. Direct structured-payload execution remains the E1.2 follow-up and no authorization boundary changed.
- **E1.2 (M)** `AgentInvocation` model with `project_id` (migration 0175), `POST /agents/{agent_id}/invoke` sync/async with semaphore, `GET /invocations/{id}`, SSE with stream ticket, retry/cancel; runs through a single-step compiled graph so leases/fencing/retry/review apply identically. **(slice 1 shipped)** Migration **0176** (0175 was taken by review_requests), `agent_invocations` with no status column. An invocation is a pipeline run with a pre-assigned id whose frozen plan keeps only the agent plus its declared dependencies (`build_workflow_plan(invocation_stage=...)`), so leases, retries, cancel and Finalize/review apply unchanged; status is read from the run. `POST /agents/{agent_id}/invoke` (async, stored subject only) and `GET /agents/invocations/{id}` (`require_invocation_access`, 404 for non-members). **Remaining:** `mode=sync` plus semaphore, model payloads, SSE with stream ticket, invocation retry/cancel routes, output projection. **(slice 2 shipped)** Invocation retry (resume same run; 409 in progress / clean finish / ceiling / config changed, last three with links.rerun; lost dispatch re-sent) and cancel through `request_cancel`. The poll view carries the invoked stage output once completed. Migration 0177 adds `agent_invocations.dispatched_at`. **Deviation:** a config-changed retry answers 409 + links.rerun instead of starting a rerun, because an invocation owns exactly one run. **Remaining:** mode=sync + semaphore, SSE with stream ticket. **(slice 3 shipped, E1.2 complete)** mode=sync only waits (the worker still runs it) on sync-eligible agents: 200 when finished within AGENT_INVOKE_SYNC_WAIT_SECONDS, 202 otherwise, 503 + Retry-After when AGENT_INVOKE_SYNC_CONCURRENCY waiters are in flight; sync on a non-eligible agent runs async. SSE: POST events/ticket (60 s, single-use GETDEL, bound to the invocation, stored as SHA-256) then GET events?ticket= (401 otherwise; fails closed; one event per public response change; ends at a terminal status, on disconnect, or after 30 min). **(M12 hardening shipped)** Manual retry locks the run, refuses cancellation, persists `retry_wait` before dispatch and fences the worker by expected attempt. Migration **0191** adds sticky invocation cancellation so a request accepted but not yet claimed by a worker can be terminally cancelled; worker creation locks and rechecks that intent. The EventSource route is mounted separately from the global Bearer/API-key dependency because its single-use ticket is the credential. Token allocation retries collisions. **Deviation:** a stream ticket is a bearer capability bound to one invocation, not to a later actor identity; authorization is checked when the ticket is issued. **Follow-up:** payloads other than a stored subject.
- **E1.3 (S)** Idempotency dependency (scoped key, body hash, 422/409 semantics, 24 h TTL, unique index). **(shipped)** Idempotency-Key on POST /agents/{agent_id}/invoke: the same key and request returns the stored invocation with 200 (failed ones too); a different request is 422; in flight is 409 + Retry-After. Scoped by user, project and route; the fingerprint covers the whole body. Redis lock for the in-flight window; migration **0190** replaces the original user/key partial index with a CONCURRENTLY-built unique partial index on `(requested_by, project_id, agent_id, idempotency_key)`, matching the Redis scope and remaining authoritative when Redis is down. Invocation creation locks the stored subject so concurrent unkeyed requests converge, while an explicit key never silently adopts an unrelated active invocation. **Deviation:** the lock has the 24 h TTL, but the stored row keeps answering for the key afterwards, because the index does not expire.
...
- **E4.2 (S)** Resolver merges env → ai_config → AgentConfig → `AgentConfigPatch`; **resolve-time** offline clamp via `apply_offline_ceiling` + `enforce_provider_policy`; unit tests for every precedence pair and for a stored cloud provider after an env flip. **(shipped)** `agent_config_resolver.resolve` (pure) and `resolve_for_project` (row + live ai_config + async residency check for endpoints with a base_url). A refused tier falls back to the global ai_config model when that is permitted, else it has no endpoint; every change is recorded in `clamps` with its layer. Stored rows are clamped to lowered attempt/timeout ceilings and the deadline before validation; a row no clamp can repair raises AgentConfigInvalid. A project block inherits the global base_url only for the same provider. Global ai_config does not floor thresholds (the section 4.1 table makes thresholds project-only). The invoke route refuses a disabled agent with 403 and an invalid stored config with 409; defect_commander is therefore not invocable until a project enables it in mode act. **(T11 shipped)** The invoke body now accepts the nested, tighten-only `AgentConfigPatch`; rejected loosenings return 422 and the full body, including overrides, participates in the idempotency fingerprint. Revision 0181 stores a credential- and base-URL-free resolved snapshot on `agent_invocations`; the worker copies it to pipeline execution metadata before any stage runs. Invocation retries and the Summary/Root Cause model routers consume that snapshot while current offline, allowlist, timeout, attempt, endpoint-residency ceilings are reapplied. A tier refused when accepted remains unavailable even if deployment policy later relaxes. A conflicting explicit override never silently reuses an in-progress invocation. **(M11 hardening shipped)** Full frozen snapshots now drive ordinary workflow execution and retry fingerprint checks, live eval-drift and environment safety clamps are reapplied on restore, Root Cause enforces its tool grant and failure cap, cache reuse cannot cross a broader tool grant, and MCP renders the sanitized snapshot. **Deviation:** T11's file list implied no schema change, but code on main had no durable invocation field capable of carrying the resolved config across API and worker processes, so the additive nullable JSONB column was required. Section 3.3's illustrative flat patch names also disagreed with the shipped nested `AgentConfigPatch` and are corrected here. **Gap:** several capability-specific model-tier, retry, timeout, review and budget settings are not yet runtime consumers; there is no durable specialist tool-call ledger or CLI config/invoke surface.
- **E4.3 (S)** Settings UI tab per agent on `SettingsPage`. **(shipped)** Built as an Agent configuration panel on Settings → AI Agents (`/settings/ai-agents`, already a QA_LEAD+ management route) rather than a new SettingsPage tab: one tab per configurable agent. Edits mode, tier, attempts/timeout (worst case shown live), budget, tool allowlist, review and override policy; saves the whole document with PUT; server refusals (422 Pydantic errors or provider courtesy strings) are listed inline; a stored row with valid=false shows its errors. The list route now also returns `tools` ({tool: permission}). Model endpoint blocks (slm/llm provider+model), escalation, thresholds and shadow sampling are not editable in the UI yet; they keep their stored or default values and remain settable through the API. The Investigator and Fixer cards project their pinned shapes from canonical AgentConfig documents.
- **E4.4 (S)** Migrate `AgentPolicy` rows into `agent_configs` (keep `shadow_runs_completed`); `agent-policies` becomes a read-only alias for one release; guard that only `agent_config_service` writes `mode`. **(T5 shipped)** Migration 0187 moves both owner-selected rows into strict agent-specific extensions, preserves all Investigator run/cluster budgets, the server-owned counter and promotion note, and preserves the Fixer runner/globs/budgets/schedule contract; it refuses unknown policy IDs and has a reconstructive downgrade. The legacy table and ORM model are removed. Deprecated legacy GETs project the pinned AgentPolicy/FixerConfig shapes; their authenticated PUTs return 405 with a canonical Location, and frontend clients use the AgentConfig endpoint. Mode and counter writes flow through `agent_config_service`; the single-writer baseline is zero. **Code-truth correction:** scheduled Fixer discovery also depended on the legacy JSON and now selects the canonical typed extension, a dependency omitted by the handover's T5 inventory. **Deviation:** the two non-registry IDs are accepted by the single-resource endpoint but remain outside the registry catalog/list route; their runtime-only fields live in typed extensions because CapabilitySpecV1 and pinned wire shapes cannot change. **Gap:** remove the deprecated aliases after one release.
...
- **E5.3 (S)** Root-cause split: SLM for category/confidence, LLM for explanation under the §5.2 rule. **(T8 shipped)** Durable pipeline runs resolve `agent.root_cause_analysis.v1` once, use its resolved SLM endpoint for the fast classifier, and preserve that category/confidence as the verdict. Confidence below `thresholds.confidence_min` or inputs spanning more than one artifact type invokes the resolved LLM endpoint with the fast classifier and caches bypassed; only explanation, actions, evidence, and tool findings are merged. The default is now SLM after E9.3. Escalation rechecks cost, call, endpoint, count, and explicit-tier constraints through `ModelRouter`; refusal or an unusable SLM result takes the deterministic rules fallback. Provenance adds classification/explanation tiers and the actual trigger to the pinned tier fields, and produced pairs use E9.3's shadow hook. **Deviation:** artifact span is computed from server-side inputs (`error_message`, `stack_trace`, history, and typed evidence artifacts), so model-authored evidence cannot trigger extra inference. Symbolic-id in-process callers retain the legacy path because they have no project configuration row. **Gap:** shadow token counts use the pipeline budget meter and are zero for unmetered direct calls; live paired sampling still occurs only on required escalations.
- **E5.4 (S)** Circuit breaker per provider+base_url in `llm_factory`; open breaker ⇒ immediate deterministic path. **(T9 shipped)** `BudgetedLLM` now checks Redis state at the actual endpoint-aware invocation boundary, before cost reservation or concurrency admission. Breaker keys hash the lowercase provider plus normalized base URL, so credentials, query strings, and raw endpoints do not enter Redis keys or metrics. Five consecutive availability failures open only that endpoint; ordinary validation/authentication errors do not count, success resets the sequence, and a Redis `SET NX` lease admits one half-open probe after 120 seconds. The global live-analysis preflight gates and breaker were removed so work reaches its configured endpoint and open circuits use the caller's existing deterministic fallback. Prometheus publishes trip transitions plus numeric state by provider and endpoint digest. **Deviation:** Redis breaker-store operations are capped at 50 ms and fail open with a warning, preserving inference when the control store alone is unavailable. Synchronous `invoke` consults the process-local open-state cache because the authoritative Redis client is async; production model paths use `ainvoke`. **Gap:** providers whose SDKs accept an implicit custom endpoint outside TestLookup's resolved `base_url` settings share that provider's `default` scope.
- **E5.5 (S)** `make dev-llm` pulls one SLM and one LLM tag; docs list tested tag pairs. **(T10 shipped)** The target now waits up to 120 seconds for Ollama and pulls the exact tested pair `qwen2.5:3b-instruct-q5_K_M` (SLM) plus `qwen2.5:14b-instruct-q5_K_M` (LLM), together with the pinned `nomic-embed-text:v1.5` embedding model. Fresh environment and application defaults reference the installed SLM and embedding tags, so legacy single-model calls work immediately. Repeated starts reuse Ollama's named-volume layers, and make-variable overrides support deliberate hardware experiments without changing repository defaults. The Unix/macOS and Windows bootstrap scripts also enable the `local-llm` profile and pull the same set. The README records sizes, intended tier use, storage requirements, and the rule that project agent configuration must select an installed exact tag. **Deviation:** Docker is unavailable on the Windows implementation host, so the story validates Compose/Make wiring and official registry tag existence rather than downloading 13+ GB locally; CI validates the static contract but does not run Ollama. **Gap:** tier selection remains project agent configuration; pulling the pair does not rewrite stored configs.

### E6 — Generic reviewer (requirement 7) — sum ≈ 14 days
- **E6.1 (M)** `ReviewerAgent` with check families 1, 2, 5, the verdict invariants, and `ReviewVerdictV1`; registered as `agent.reviewer.v1`. **(T15 shipped)** `ReviewerAgent` accepts a strict, bounded bundle of reviewed step outputs plus authoritative reference, numeric, mode, and tool-policy context. Family 1 validates the registered Pydantic output contract (including today's `agent_contracts` wire adapter), grounds test/cluster/artifact identifiers, and binds labelled prose numbers to state facts within a configured tolerance. Family 2 maps the existing Summary, Root Cause, Release Risk, and Decision Report consistency checks into typed `ReviewCheckV1` and `ReviewDisagreementV1` rows. Family 5 rejects undeclared tools and mutating tool use below `act`, and flags any non-`act` mutating action that has advanced beyond `proposed`. `ReviewVerdictV1` is strict and fail-closed: every continuing verdict requires families 1, 2, and 5 for every reviewed step; `pass` additionally requires no failed check or blocking disagreement, non-high hallucination risk, and second-model agreement ≥ 0.7 when present; every non-pass verdict requires human review. The catalog publishes resolved input/output JSON Schemas and deterministic zero-cost routing metadata without changing `CapabilitySpecV1`; runtime results add the standard `agent_contracts.reviewer` provenance envelope around that public verdict. Watched reviewer-source attestations now hash normalized UTF-8 text after CI exposed that raw Windows CRLF bytes did not match the Linux checkout of the same source. **Deviation:** the capability is declared `on_demand` rather than added to a built-in fixed graph; E3 will make the documented `reviews:[...]` workflow attachment executable. **Gap:** direct invoke supports planned fixed-graph stages only, so this slice exposes the catalog contract and callable agent service but not an HTTP execution path. Families 3-4, Supervisor handling, incident emission, output downgrade application, and shared `step_llm_budget` remain E6.2-E6.4.
- **E6.2 (S)** Self-consistency claim extraction (family 3) on SLM. **(T16 shipped; M13 budget correction shipped)** The reviewer re-asks the reviewed step's resolved SLM/LLM tier to extract reference ids and numeric facts, strips hidden-reasoning keys before prompt construction, and diffs the typed response against the authoritative evidence snapshot. Missing endpoints and malformed responses fail into a warning check and cap the verdict at `pass_with_flags`. **Code-truth correction:** the first T16 implementation also charged family 3/4 checks to `step_llm_budget`, but §6.2 defines that counter only for producer escalation and reviewer retry loops. Family checks now rely on the existing run-level atomic cost and call authority instead.
- **E6.3 (M)** Second-model agreement (family 4) with heterogeneous model preference; config `review.second_model_check`. **(T16 shipped; M13 contract correction shipped)** When enabled alongside the auto-reviewer, a distinct configured LLM returns a bounded agreement score plus unsupported and blocking claims. Agreement below 0.7 or non-blocking unsupported claims yields `pass_with_flags` and pins human review; it is not a blocking contradiction. A blocking unsupported claim requests the single supervised retry and rejects after that retry. The generator and reviewer prompt versions are independent. **Deviation:** today's strict config has one LLM endpoint, so the reviewer guarantees a different provider/model identity and uses a different family when that endpoint supplies one; selecting among multiple LLM-family candidates remains a future additive config change.
- **E6.4 (S)** Supervisor handling of `retry/reject/pass_with_flags`; shared `step_llm_budget`; mutation-tested checks (orphan ids, contradicted numbers, unsupported claims). **(T16 shipped; M13 runtime authority shipped)** A deterministic supervisor revalidates `ReviewVerdictV1` fail-closed and emits typed `continue`, `retry`, or `finalize` instructions. Retry names the reviewed steps and pins `tier_override=llm`; a second retry or exhausted budget becomes `finalize`, `failed`, `validation_failed`; flags continue with `requires_human_review=true`. `StepLLMBudget` is serializable and its initial ceiling equals `min(run calls remaining, max_escalations + max_iterations)`. Family-5 blocking findings emit a `reviewer_incident` decision event. The custom runtime stores that budget by workflow step id, passes the same counter through Summary or Root Cause model escalation and its reviewer retry, and applies the supervisor's `llm` tier override on the single retried producer execution. Custom runtime review flags prevent automatic pass, create an evidence-bound human review subject even without a report-producing stage, and persist the verdict, supervisor route, and budget snapshot. Regression and mutation checks cover orphan ids, contradicted numbers, unsupported claims, shared-budget exhaustion, override propagation, one-retry handling, and invalid supervisor routes. **Deviation:** budget and override propagation currently target the two tier-routed producer capabilities, Summary and Root Cause; deterministic reviewed capabilities accept the state fields but do not route models. **Gap:** no remaining E6.4 runtime-authority gap is known; the broader M13 G4 replay/publish authority findings remain under E9.5.

### E3 — User-customizable workflows (requirement 3) — sum ≈ 26 days
- **E3.1 (M)** `workflow_definitions` table (migration 0177) with versioning + publish immutability; CRUD, validate, fork, publish; built-ins read-only (405). **(T17/E3.1 shipped)** Migration **0183** is the next linear head because 0177 already belongs to invocation dispatch timestamps on `main`. Project-scoped CRUD, strict structural validation, fork, and publish routes are protected by project membership and QA Lead mutation guards and record activity events. The three current planner stage sequences are exposed as global read-only templates; PUT/DELETE return 405, and fork creates project version 1. Published custom JSON is never updated: PUT allocates the next draft version while holding a transaction-scoped advisory lock. **Deviation:** the architecture's reserved migration number was stale. **Gap:** validation is explicitly structural; E3.2 still owns registry/dependency/tool/condition/cycle compilation and authoritative built-in graph diffing, E3.3 still binds `workflow_id@version` and plan hash to runs, and E3.4-E3.5 remain unstarted.
- **E3.2 (L)** `WorkflowCompiler`: registry validation, dependency closure, cycle rules, capability-permission ≤ mode, step tools ⊆ config allowlist, typed JSON condition AST evaluator with limits and enum checks (fuzz tests), emits `StateGraph`; three built-ins expressed as definitions and diff-tested against today's compiled graphs. **(T17/E3.2 shipped; M13 hardening shipped)** `agents/workflow_compiler.py` rejects unknown or non-top-level capabilities, registered capabilities without a concrete workflow executor, dependencies that cannot complete before their consumer, ordinary-edge cycles, invalid reviewer targets, project config/mode/tool violations, and non-exhaustive ordered branches. Its JSON condition AST accepts only bounded boolean composition and typed comparison/count/null/boolean/else leaves over a published state projection or frozen config references; enum values, depth 6, 32 leaves, and `in` lists of 50 are enforced before publication. Missing runtime numeric facts now make relational branches false rather than raising. Explicit back-edge loops use compiler-owned counter nodes and stop exactly at `max_iterations`; reviewer retry loops are uniquely declared and bounded to one. The workflow validate endpoint reports semantic results, and evaluation/publish fail with 422 before G4 work when compilation is invalid. Offline, deep, and live definitions describe the current conditional and parallel topologies, with diff tests against the hand-built graphs. **Code-truth correction:** finalization is a fenced post-graph transaction, not a registered agent node, so terminal paths compile to LangGraph `END`. **Deviation:** disabled capabilities remain structurally compilable because the live deep template contains feature-gated, default-disabled `defect_commander`; permission is enforced whenever a config enables execution. **Gap:** reviewer artifact identifiers and full cross-agent sharing of the step LLM budget remain open.
- **E3.3 (S)** Runs record `workflow_id@version` + plan hash; checkpoint restore refuses across versions. **(T17/E3.3 shipped; M13 hardening shipped)** The pipeline trigger accepts an additive workflow id/version selection and resolves only immutable published versions, defaulting compatibly to `offline@1`. Run creation freezes the exact definition, canonical definition digest, compiled plan digest, deadline, retry/review policy, and safe project agent-config documents in `execution_metadata`; terminalization preserves that authority. Both first execution and same-id resume compile the frozen snapshot. One runtime-authority digest now binds project, workflow id/version/ref, definition, plan, configured policies, and resolved endpoints, so coordinated metadata edits cannot preserve a runnable snapshot. Step-id executor binding supports repeated named capabilities with task-local durable stage identity. Custom step tool declarations narrow the frozen project allowlist. Reviewer steps receive declared upstream outputs and authoritative run facts, derive their retry count from the compiler loop, and stop the graph on final rejection. **Deviation:** no columns or migration were added because `execution_metadata` is already the immutable replay-authority store. **Gaps:** reviewer artifact references remain empty, and the step LLM budget is shared across reviewer retries but not yet every producing-agent escalation.
- **E3.4 (M)** Workflow editor UI (list/fork/edit with schema validation/preview graph/publish, shows `eval_coverage`) on `AgentWorkflowPage`. **(T17/E3.4 shipped)** The project-scoped `/agents/workflows` editor lists built-in and custom versions with evaluation evidence; gives QA Leads fork, JSON edit, bounded topology preview, server-side semantic validation, G4 evaluation, and publish controls; and keeps built-ins plus historical versions read-only. Publishing always revalidates the stored version, while a measured failure requires explicit regression acceptance and a reason. The active agent pipeline links to the editor through the shared project-scope guard. Six asserted mutations cover immutability, coverage, validation-before-publish, fan-in preview, PUT semantics, and regression acceptance. **Code-truth correction/deviation:** `App.tsx` explicitly treats the old `AgentWorkflowPage` as a dormant design reference and serves the operational `AgentStatusPage` at `/agents`, so replacing that live monitoring page would contradict the code; the editor therefore has a dedicated route linked from the live page. **Gap:** condition ASTs and other schema details are edited as validated JSON rather than through dedicated form controls; E3.5 still owns the built-in/compiled guard and final workflow documentation.
- **E3.5 (S)** Guard `workflows.builtins-match-compiled` + docs. **(T17/E3.5 shipped)** The absolute pure-standard-library guard pins collection of the executable compiler regression, exact `offline`/`deep`/`live` parametrization, real runtime executors, and full node/edge/conditional-branch topology comparison. Fixture self-tests catch deleted built-ins, fake executors, narrowed assertions, and skipped coverage. `architecture/WORKFLOWS.md` documents authoring, validation, evaluation, immutable publication/execution, API roles, and parity checks. **Deviation:** the lightweight quality job statically validates the parity regression instead of importing LangGraph; the required backend suite performs the real graph comparison. **Gaps:** advanced schema fields remain JSON-edited in the UI, and reviewer artifact references await the canonical artifact-id state projection.
...
- **E9.3 (M)** G2 tier comparison: paired harness run, CI-based non-inferiority, `POST /api/v1/ai-eval/tier-comparison`, `PUT agent-configs` hook (refuse regressing downgrade; allow upgrade on insufficient); shadow sampling with `shadow.sample_rate` and token budget on the `default` queue; shadow pairs stored as labelling candidates. Migration 0178: `ai_eval_shadow_pairs`, `ai_eval_gate_runs` manifest columns. **(T14 shipped)** Paired incumbent/candidate outputs are scored against the same frozen capability labels. The response records Wilson accuracy intervals, a paired 95% difference interval, perfect-validity smoke criteria below n=100, hard non-inferiority at n≥100, and cost/latency deltas. The project-scoped POST persists a typed G2 gate row; agent-config PUT refuses a failed comparison and refuses an insufficient downgrade while permitting an insufficient upgrade. Stable shadow sampling enqueues already-produced pairs on `default`; a transaction advisory lock makes the per-project/agent/day token cap hard, and stored pairs remain `pending` labelling candidates. **Deviation:** migration 0180 was used because origin/main already assigns 0178 to invocation idempotency and 0179 to agent configs; subsequent planned migrations move to 0181+. Summary and root-cause analysis now call the router and shadow hook through E5.2/E5.3. **Gap:** live shadow pairs arise only on executions that produce both tiers and pass sampling and token limits.
- **E9.4 (M)** G3 reviewer quality (after E6.1 merges, not parallel): per-class × per-family recall, clean-corpus false-flag rate, false-omission rate from human outcomes; auto-disable rule with the n ≥ 30 and Δrecall ≥ 0.10 guards. Migration 0181+: `ai_eval_reviewer_quality`. **(T18 slice shipped)** `POST /api/v1/ai-eval/reviewer-quality` accepts strict, time-stamped labelled mutation, clean-output, and human-outcome observations; reports Wilson intervals for every mutation class × family 1–5 cell; and gates on ≥0.95 semantic family-3/4 recall, ≤0.10 clean false flags, and ≤0.10 false omissions with a 20-sample smoke floor. Migration 0182 stores raw observation batches separately from scheduled rollups. The existing 04:00 UTC eval task aggregates a trailing 30-day window and may disable `review.second_model_check` only when each of the three semantic classes has n≥30 and family 4's aggregate recall advantage over deterministic families 1/2/5 is <0.10; the change uses `agent_config_service.put_config` and is audited. **Deviation:** 0181 was already used by T11, so the next head is 0182. **Gap:** E6 is not attached to built-in or custom graphs yet, so runtime reviewer observations and reviewer-verdict-to-`ReviewRequest` outcome joins have no live producer; the QA-lead endpoint supplies labelled batches until E3 wires those sources.
- **E9.5 (M)** G4 workflow evaluation: replay cache keyed by `(agent_id, prompt_version, input_hash)`, `unmeasured` on miss, coverage metric, `POST …/workflows/{id}/evaluate`, publish refusal unless `accept_regression` + reason. Migration 0182+: `workflow_replay_corpus`. **(E9.5 shipped; M13 authority hardening shipped)** The project-scoped dry-run caches stage outputs under the exact three-part key and compares candidate definitions with the latest 20–100 runs for plan-verification pass rate, degradation, reviewer rejection, cost, and latency. `input_hash` binds the runtime's validated stage-input receipt to a per-step authority digest covering the executable graph and policies, a behavior-only compiled-plan projection, project feature and budget inputs, frozen resolved config, a credential-free endpoint fingerprint, prompt versions, and recorded runtime versions. Workflow id, version, ref, name, and description remain provenance and do not prevent behavior-identical forks or later versions from reusing exact evidence. Missing or tampered receipts are unmeasured; duplicate stage attempts select the terminal latest attempt deterministically. A historical output with different authority is explicitly reported and treated as a publish-blocking regression requiring `accept_regression`, a QA Lead's reason, and the exact fresh evaluation-manifest checksum. The manifest includes a digest of every replay receipt/output checksum, status, review fact, baseline, cost, and latency used by the decision. Evaluation acquires global then project authority locks and reads global AI settings once, feature flags fresh, plus project configs, policy, and drift pins from that stable snapshot; all corresponding writers share those lock domains. Corpus population is serialized by project and base workflow. The editor and publish both use the fixed authoritative 100-run window, and publish re-runs G4 on every non-idempotent attempt instead of trusting a draft verdict. Arbitrary step `model` metadata is semantically refused until runtime supports a typed override. Evaluation refuses published versions. Publish requests bind the selected version and canonical definition digest; stale requests return 409, while an exact repeat is read-only. **Code-truth correction:** the original cache key's `input_hash` covered only project, test run, and step, so the earlier shipped text overstated model/config/topology measurement. **Deviation:** migration 0184 was used because origin/main already assigns 0182 and 0183; no schema change was needed because hashes fit the existing cache and gate manifests. **Gaps:** candidate reviewer, cost, and latency values still come from exact-authority historical outputs; optional live candidate inference and project-default enforcement remain open.
- **E9.6 (S)** G5: CI-based drift over the nightly `AIEvalRun` rows + review reason_code rates + incident marks; weekly beat; drift ⇒ `ReviewRequest(kind=eval_drift)` and the capability-level pin (block downgrade, disable auto-only acceptance). **(E9.6 shipped)** A Monday 06:00 UTC beat compares the latest seven days with the preceding seven. Accuracy, capability-scoped review acceptance/reason rates, and release-risk incident-after-GO rates use Wilson 95% intervals; only non-overlap is drift, while either window below 20 observations is `measured: false` and creates no action. A degrading signal creates one pending capability `eval_drift` review. That live review is the durable pin: config PUT and invocation overrides refuse quality-tier downgrades, and resolution forces `human_required` with automatic model review disabled until a QA Lead accepts or rejects the review. **Deviation:** `AIEvalRun` predates projects and capability ids, while `ReviewRequest.project_id` is mandatory. The global nightly task-type signal is therefore applied only to projects with an explicit config row for the mapped capability; the composite subject id includes project and capability because the existing live-subject index is global. **Gap:** projects using only default configs cannot receive a project-scoped pin until eval rows gain project attribution (or the review model gains a system scope). Existing review requests usually have `capability_id = NULL`, so their reason-code signal remains unmeasured until producers stamp that optional field. The runtime has no auto-reviewer-only acceptance path—human review is already mandatory—so the resolver clamp disables the optional model reviewer as the available fail-closed equivalent.
- **E9.7 (S)** Provenance: `eval_manifest_checksum` frozen into `execution_metadata` (after E7.1); `GET /api/v1/ai-eval/gates/{checksum}`; dashboard flag for unresolvable checksums. **(E9.7 shipped)** Both ordinary and Investigator pipeline constructors freeze the current passing attestation checksum, and terminal metadata reconstruction preserves it. The checksum resolver returns a database-backed `AIEvalGateRun` or an immutable bundled attestation; the AI Evaluation Dashboard reports exact seven-day stamped/resolved/missing counts and flags any missing or unknown checksum. Prompt attestation now writes and validates a checksum-addressed archive entry, so a later release cannot make the preceding manifest unresolvable. **Deviation:** the active source-review attestation has `eval_gate_run_id = null`, contrary to the original premise that every admitting gate is database-backed; bundled immutable manifests cover that existing deployment mode without inventing a database row. **Gap:** pipeline rows created before E9.7 have no checksum and remain visibly unresolved until they age out of the seven-day window.
- **E9.8 (S)** Eval on the eval, scoped to the delta over the 22 existing harness tests: per-rule mutation of each scorer in `agent_eval_harness` and `ai_eval_service`. **(E9.8 shipped)** Thirteen focused rule probes now exercise coherence's three failure modes, completeness and actionability boundaries, accuracy, calibration and sample floors, plus every task scorer and the unknown-task dispatch fallback. A CI-owned mutation harness reverses or disables 29 individual rules, asserts each source replacement applies exactly once, and requires every mutation to fail the focused suite. **Deviation:** the out-of-range coherence probe uses an unvalidated model copy because normal `AgentEvalSample` construction clamps confidence to 0–100 before the defence-in-depth scorer runs; no production scorer change was needed. **Gap:** new scorer rules must add both a focused probe and an exact-source mutation or the suite cannot assess them.
- **E9.9 (S)** Release outcome source: manual "mark incident / rollback" action on a release (API + UI), recorded in the activity ledger, joined by G5. **(E9.9 shipped)** QA Leads can append a reasoned incident or rollback through `POST /api/v1/releases/{release_id}/outcomes`; each row records release, project, actor and time, and the same transaction emits `release.outcome_marked`. Release detail returns the newest 50 outcomes and the Releases UI provides both actions plus their history. Migration 0185 adds the append-only source and indexes for release history and G5's project/time windows. **Gap:** E9.6 will consume `list_project_outcomes_between` to calculate incident-after-GO drift; this story supplies and exposes the source without implementing G5 early.
- **E9.10 (S)** Label leakage rules: manifest stamp on feedback/review-derived rows, gate exclusion of same-manifest rows, 14-day holdout; test that a dataset built from feedback on manifest M cannot gate M. **(E9.10 shipped)** `ai_feedback` and `review_requests` now retain the checksum of the eval manifest that produced the graded subject; AnalysisAgent copies the pipeline's frozen checksum into its routing audit, report reviews copy it from the proposing run, and drift reviews stamp the active manifest. Feedback-built dataset items freeze that checksum and label time after a 14-day holdout. The agent-stack release gate rechecks every stored feedback/review-derived item and excludes labels from the candidate manifest, recent labels, and derived labels with missing provenance; static golden items remain eligible. Migration 0186 adds the two nullable, checksum-constrained columns without new indexes because the bounded feedback query already uses its created-at index. **Deviation:** rows created before this story and direct analyses with no admitting pipeline remain NULL and fail closed for release gates rather than being guessed or backfilled. **Gap:** the runtime has no review-derived dataset builder yet; review rows are stamped now and the gate already recognizes future `review_id`/`label_source=review` items.

### E2 — Hardening and observability — sum ≈ 8 days
- **E2.1 (S)** OTel spans per step with `gen_ai.*` attributes; Grafana panels. **(shipped)** The workflow's checkpoint wrapper now owns one canonical `testlookup.agent.step` span for every executed, skipped, restored, or failed LangGraph node. Spans carry `testlookup.agent_id`, the capability's default `tier`, the pipeline `attempt`, `gen_ai.request.model`, and aggregated `gen_ai.usage.input_tokens` / `gen_ai.usage.output_tokens`; the shared `BudgetedLLM` boundary supplies the actual provider/model and provider-reported usage, including bound and structured-output clients. Grafana provisions a stable Jaeger UID and exposes recent and failed step-trace panels. **Code/document correction:** main already had optional `BaseAgent.mark_stage_running` spans, while the backlog premise implied step tracing was absent; several nodes never call those lifecycle hooks and their attributes used private names, so they did not satisfy this story. **Deviation:** those existing lifecycle spans remain as nested detail spans for compatibility; the workflow wrapper is the complete step boundary. **Gap:** escalation within a step is visible through its final actual model, but `tier` remains the capability default because routing sources are eval-attested and E2.1 does not alter their behavior or attestation.
- **E2.2 (S)** New counters from §10 with emission tests. **(shipped)** Added Prometheus counters at the workflow-step, state-machine, retry, review-state, and successful tier-escalation boundaries. Emission tests cover success and error invocations, actual edges but not same-state calls, bounded retry reasons, every review state, both escalation call sites, and the existing circuit gauge; an 11-mutation harness proves those assertions detect missing or mislabelled emissions. **Code/document correction:** main already had the E5.4 endpoint-scoped `testlookup_llm_circuit_breaker_state{provider,endpoint}` rather than the provider-only gauge named by §10. The endpoint label is required to isolate independent endpoints for one provider, so E2.2 retained and tested it and corrected §10 instead of adding a weaker duplicate. **Deviation:** `agent_retry_attempts_total.agent` is the pipeline workflow type because pipeline-level retries have no single failed capability identity; reasons are bounded to `execution_failure|lease_expired` to avoid exception-text cardinality. **Gap:** no dashboards or alert rules consume these counters yet; E2.3 owns alerting.
- **E2.3 (S)** DLQ list/replay endpoints; alert rules (`in_progress` age beyond deadline + grace, DLQ depth, `pending_review` age); each alert has a **positive** test that injects the condition and asserts it fires and clears. **(shipped)** Added the canonical instance-admin list/replay routes over the existing Redis LIST and STREAM stores. Replay accepts only the three server-authored Celery task shapes, claims an entry against concurrent operators, and deletes it only after broker acceptance; live-event entries and unknown task names fail closed. The pipeline DLQ payload now preserves its required project and selected workflow, which main omitted. Scrape-time gauges query the oldest public-`in_progress` run (`pending|running|retry_wait`), total depth across both Redis DLQ stores, and oldest pending review. Prometheus alerts at deadline + configurable 300-second grace, depth > 0, and review age > 24 hours; positive tests drive every rule through firing and cleared values. A ten-mutation harness covers the safety and alert thresholds. **Code/document correction:** main already had the read-only `/api/v1/admin/maintenance/dlq` compatibility endpoint, so the earlier “no DLQ router exists” premise was stale; it remains available. **Deviation:** replay is limited to Celery entries written by `_send_to_dlq`; live-event stream recovery and the separate `persist_live_session` LIST require source-specific workflows and are inspection-only here. **Gap:** replay is broker-at-least-once if Celery accepts a task and Redis entry deletion then fails; the retained 60-second claim prevents an immediate duplicate but cannot make two systems atomic.
...
- `GET /openapi.json` lists every registry capability under `/api/v1/agents/{agent_id}/invoke` with a per-agent input schema; a Postman run of the generated collection returns `202` then `passed` for every independently invokable sync-eligible agent and `202` then `completed` for every independently invokable report-producing agent on the seed project. Workflow-only stages remain visible in the catalog with `invokable=false` and are executed through their surrounding workflow.
- `SELECT DISTINCT status FROM agent_pipeline_runs` returns a subset of the six internal states; the public API never returns anything but the four; the "stuck beyond deadline" alert fires when an active run is older than its deadline plus grace, and clears after reaping and recovery move the run to a terminal state (positive test, not "zero alerts on seed traffic"). Lease expiry is the reaper trigger; `retry_wait` deliberately remains an active state and does not itself clear this age alert.
...
**(T22 shipped, with live findings and gaps)** The generated-contract probe passed all eight independently invokable sync/report capabilities on the seeded Podman stack: four non-report runs reached `passed/not_applicable` and four report runs reached `completed/pending_review`. Internal and public status projections stayed within their six- and four-value contracts. A live pause, lease expiry and reap exposed the only unfenced finalization write; finalization now selects by fencing token under `FOR UPDATE`, and the repeated probe reached `retry_wait(1) → running(2) → completed(2)` while the stale token updated zero rows. With review enforcement enabled, pending notification text was withheld, release readiness returned `PENDING_REVIEW`, and intelligence export now returns an audited 409; accepting as a different, non-synthetic QA lead moved the run to `passed`, exposed the accepted export, and recorded reviewer identity only in the audit log. The overdue alert was positive during the injected old active run and zero after recovery terminalized it. Static eval checks passed for watched-source attestation enforcement, corpus floors, the summary G2 rule, reviewer per-class recall with `n`, manifest resolution, and reasoned workflow-regression refusal. The current bundled attestation is `mode=source_review` and explicitly lacks candidate-inference recordings for reviewer prompts, and the 2 GiB local Podman machine could run only the offline lite stack; therefore the fresh inference-backed attestation and live SLM-versus-LLM comparison remain release-evidence gaps. Full evidence and exact deviations are recorded in `architecture/verification/AGENTIC_LIVE_DOD_2026-09-15.md`.
