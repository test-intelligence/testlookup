...
`make quality-gate` runs `scripts/quality_gate.py`, which enforces **46 guards**.
...
fail CI. The other 28 ship at zero with **no baseline file at all** — those are
...
| `backend.analytics-epoch-bump` † | a function that commits an analytics mutation — a registered base writer in `_ANALYTICS_MUTATION_BASES` (run/release/live/deletion/reset writers, plus the defect, active release-gate policy, quarantine-request, AI-analysis and failure-cluster writers the cached dashboard and hours-saved model read), a source-discovered write (`update`/`delete`/`insert` of `TestRun`/`TestCase`, a new `TestRun`/`TestCase` row, an assignment to `status`/`primary_release_id`/`primary_suite_name`/`is_active` on such a row or `Project`), or a caller that reaches one without committing — and does not `await` `bump_analytics_epoch` / `bump_analytics_epochs` / `invalidate_analytics_cache` (resolved by import to `app.services.cache_service`) **after** the commit; or bumps before it, in `except`/`finally`, in a dead branch, or through `create_task`. A `get_db` route handler commits again when it returns, so it must commit explicitly before its bump. backend/scripts is not scanned | Analytics cache keys carry a per-project epoch (VIZ-212); a mutation that commits without bumping it leaves every cached report stale until TTL. After the commit: `await bump_analytics_epoch(project_id)`, or `bump_analytics_epochs(ids)` once per project for a sweep. A new primitive writer of what analytics read goes in the registry. A path that changes nothing analytics reads opts out with `# analytics-epoch: none — <reason>` |
